...
NAMELIST /bio_params/ mu_max_day, kn, tslope_cold, tslope_hot, mutation_width, mortality, w_mig, w_sink, tau_trans, use_akinetes
...
PRINT *, 'DEBUG: aorg_init=', aorg_init, ' phyt_init=', phyt_init, ' nyears=', nyears

OPEN(12, FILE='fort.12', FORM='UNFORMATTED', STATUS='UNKNOWN')
...
#endif
#ifdef SEASONAL_TEMP
   ! Medwed 1987 Initialization
   phyopt(i)=rand_normal(15.3d0,0.86d0)
#endif
...
#ifdef SEASONAL_TEMP
    ! seasonal cycle
    temp=15.0d0+5.0d0*sin(2.d0*pi*time/secyear)
    
    ! warming trend
    temp=temp+1.5d0*(time/endtime)

    ! Experiment C: Stochastic Heatwaves
    ! Add +4.0 degC spike if random number < 0.01 during summer (days 150-240)
    day_val = dmod(time, secyear) / secday
    if (day_val .ge. 150.0d0 .and. day_val .le. 240.0d0) then
       call random_number(rand_val)
       if (rand_val .lt. 0.05d0) then ! 5% chance per step (hourly) -> Frequent spikes
          temp = temp + 4.0d0
       endif
    endif
...
DO i=1,M2max
   IF(iliv(i).eq.1) THEN
    ...
    IF(temp.le.phyopt(i)) THEN
       growthmean0=growthmean0+exp(-((temp-phyopt(i))/tslope_cold)**2)
    ELSE
       growthmean0=growthmean0+exp(-((temp-phyopt(i))/tslope_hot)**3)
    ENDIF
